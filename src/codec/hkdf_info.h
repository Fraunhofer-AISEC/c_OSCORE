...
#ifndef NONE_HKDF_INFO_H
#define NONE_HKDF_INFO_H
...
/**
 * Returns the length of the encoded HKDF Info as defined by OSCORE
 * @param id "Sender ID or Recipient ID when deriving keys and the empty byte string when deriving the Common IV"
...
/**
 * Encodes the HKDF Info as defined by OSCORE into the out-array
 * @param id "Sender ID or Recipient ID when deriving keys and the empty byte string when deriving the Common IV"
 * @param id_context ID Context
 * @param aead_alg AEAD Algorithm
 * @param type type of operation this HKDF Info is going to be used for
 * @param out out-array. Must have a length of exactly the value returned in the out-parameter by `hkdf_info_len`.
 * @return OscoreError
 */
OscoreError create_hkdf_info(array id, array id_context, enum aead_algorithm aead_alg, enum derive_type type, array out);

#endif //NONE_HKDF_INFO_H
